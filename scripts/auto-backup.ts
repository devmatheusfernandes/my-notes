import { initializeApp, cert, getApps, AppOptions, ServiceAccount } from "firebase-admin/app";
...
function initFirebase() {
  if (getApps().length === 0) {
    const options: AppOptions = {
      credential: cert(FIREBASE_CONFIG as ServiceAccount),
...
async function performBackup(userId: string, refreshToken: string, db: Firestore, storage: Storage) {
  try {
    ...
    const notesWithFiles = await Promise.all(rawNotes.map(async (note) => {
      if (note.type === "pdf" && note.fileUrl) {
        try {
          ...
          console.warn(`[${userId}] Failed to download PDF for note ${note.id}: ${error.message}`);
        }
      }
      return note;
    }));
    ...
    const error = err as Error;
    console.error(`[${userId}] Backup failed: ${error.message}`);
  }
}

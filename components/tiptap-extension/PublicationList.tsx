...
export interface PublicationListProps {
  items: JwpubMetadata[]
  command: (item: JwpubMetadata) => void
}

export interface PublicationListRef {
...
export const PublicationList = forwardRef<PublicationListRef, PublicationListProps>((props, ref) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl overflow-hidden min-w-[280px] max-h-[320px] flex flex-col"
    >
      ...
      <div className="overflow-y-auto p-1 py-1.5 no-scrollbar">
        {props.items.length > 0 ? (
          props.items.map((item, index) => (
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${index === selectedIndex
                ...
                <Book className={`w-4 h-4 ${index === selectedIndex ? 'text-white' : 'text-zinc-500'}`} />
              </div>
          ...
          <div className="p-4 text-center">
            <p className="text-xs text-zinc-500">Nenhuma publicação baixada.</p>
          </div>
        )}
      </div>
